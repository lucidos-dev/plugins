...
| [`token-cost`](./token-cost/) | Real-time token usage and running cost across every model call the workspace makes. Live totals that tick as agents stream, cache-hit share, context-size distribution, and per-model/per-producer cost from a pricing table you edit in the app's own Settings tab. |
